...
from sqlalchemy import and_, delete, join, or_, select, update
from src.schemas.enums import TenderServiceType, TenderStatus
from src.db.models.models import Bid, BidHistory, Employee, Feedback, Organization, OrganizationResponsible, Tender
...
async def get_bids_by_tender(
    session: AsyncSession, tender_id: str, username: str, limit: Optional[int] = None, offset: Optional[int] = None
) -> List[Bid]:
    ...
    access_query = (
        ...
        .filter(
            (Tender.id == tender_id) & (
                (Tender.creator_username == username) | 
                (Employee.username == username)
            )
        )
    )
    ...
    bids_query = select(Bid).filter(Bid.tender_id == tender_id).limit(limit).offset(offset)
...
async def make_feedback(session: AsyncSession, bid_id: str, bid_feedback: str, username: str):
...
async def get_reviews(session: AsyncSession, tender_id: str, author_username: str, request_username: str, limit: int, offset: int):
    stmt = (
        select(Employee)
...
async def submit_decision(session: AsyncSession, bid_id: str, new_desicion: str, username: str):
    bid = await get_bid_by_id(session, bid_id, username)
